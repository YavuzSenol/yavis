...
**Read the handover doc first.** It lives **in OneDrive**, in the **private** area (privately synced across all machines — NUC/Laptop/Büro), not in this repo: `%OneDrive%\YSC-Yavuz\Claude\UEBERGABE_YAVIS.md` (on this account the concrete path is `C:\Users\senol\OneDrive - Yavuz Senol Consulting\YSC-Yavuz\Claude\UEBERGABE_YAVIS.md`). It sits next to the other private YAVIS files (`login.local`, `logs_dats.txt`, `_starface_zugang.txt`). It is the living handover doc (current feature status, open tasks, DB schema, endpoints) — keep it updated there. **It is deliberately NOT in Git** because it contains real personal data (candidate/contact names + private emails) and this repo is public. ⚠️ Moved 27.07.2026 out of the old `YSC-Media\YAVIS-Doku\` location (YSC-Media has employee access; the doc has personal data → belongs in private `YSC-Yavuz`). If an old copy still exists at `C:\Claude\UEBERGABE_YAVIS.md` or under `YSC-Media\YAVIS-Doku\` on any machine, it is stale — delete it and use the `YSC-Yavuz\Claude` one. Rule going forward: only personal-data-free docs (like `KONSISTENZ.md`) belong in the public repo; anything with names/emails/credentials syncs via OneDrive.
