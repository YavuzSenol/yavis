...
- **Kandidatenprofil zusenden** — Button „📤 Profil senden" am Kandidaten (`profilSendenDialog`): Empfänger per Ansprechpartner-Suche oder E-Mail; **persönliche Anrede** „Guten Tag Herr/Frau {Nachname}," (Anrede-Dropdown, Nachname autom. aus Kontakt); **Vorlagen-Dropdown** „Profil mit Namen (Standard)" / „Profil anonymisiert"; formatiertes Profil im Text; **PDF-Anhang** aus den Kandidaten-Dokumenten wählbar (vorausgewählt bei „profil" im Namen → das selbst erstellte Kandidatenprofil, NICHT der Roh-CV), via Storage-Download → base64 → Graph-`fileAttachment` (≤3 MB). Versand via Graph + Protokoll (📎-Marker im Betreff). **Vorlagen zentral verwaltbar:** Einstellungen → Profil-Vorlagen (CRUD, `oeffneProfilVorlagen`), localStorage `yavis_profil_vorlagen`, Platzhalter `{Gruss}`/`{Name}`/`{Profil}`/`{MeinName}`, `anonym`-Flag je Vorlage; Dropdown + „⚙️ verwalten"-Link im Sende-Dialog.
- **Firmen-Suche: Stadt-Filterfeld** + **konfigurierbare Suchfelder** (🔧-Filter-Knopf, ein-/ausblendbar, `kFilterSichtbar` → localStorage `kunden_filter`; Felder mit `data-filter`-Attribut; `applyKFilter`/`toggleKFilter`).
- **Einstellungen gegliedert in 5 Tabs** (Darstellung / Konto / E-Mail & Vorlagen / Listen / Daten & System) statt langer Scroll-Liste. Abschnitte tragen `class="setSec" data-cat=...`, Umschaltung `setEinsTab(cat)`, gemerkt in localStorage `yavis_eins_tab`.
